...
std::optional<std::string> sendMessage(std::uint16_t vid, std::uint16_t pid, std::string const &message,
                                       std::uint16_t in, std::uint16_t out, int timeout) {
    try
    {
        // Initialization
        std::wcout << L"Initializing Device object and grabbing endpoints..." << std::endl;
        std::list<std::shared_ptr<LibUSB::Device>> deviceList = LibUSB::LibUSB::FindDevice(vid, pid);
        ...
        std::shared_ptr<LibUSB::Endpoint> pBulkOUTEndpoint = pInterface->getEndpoint(out);		// Note that endpoint 0 is ALWAYS the default control endpoint, no matter the interface.
        std::shared_ptr<LibUSB::Endpoint> pBulkINEndpoint  = pInterface->getEndpoint(in);
        ...
        else
        {
            ...
            {
                errorStr << L"Data IN not received! (Reason " << pTransferIN->Result() << L")";
